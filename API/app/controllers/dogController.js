...
const dogController = {
    ...
    // On veux voir la fiche d'un chien
    findOneDog: async (request, response) => {
        try{
            const dogId = parseInt(request.params.id);
            const dog = await Dog.findOnePet(dogId);
            if(dog){
                response.render('oneDog', {
                    dog
                });
            }else {
                response.status(404).json(`le chien numéro ${dogId} n'existe pas`)
            }
        }catch(error){
            console.trace(error)
            return response.status(500).json(error.toString());
